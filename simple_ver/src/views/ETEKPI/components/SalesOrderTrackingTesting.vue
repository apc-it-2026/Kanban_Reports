...
const getBackgroundColorCut = (
  ...
  const weekEnd:any = plannedWeek ? new Date(plannedWeek) : null;
  const today = new Date();
  debugger ;  
  // If completed = scheduled → green
  if (completedQty >= totalQty && totalQty!==0) { 
    return `${type}-greenbackground`; 
  } 
  ...
  debugger ; 
  console.log(weekEnd) ; 
  if (completedQty >= 0 && today >= weekEnd && weekEnd != null) {
    return `${type}-redbackground`;
  } 
    return `${type}-blackbackground`;
};  


const getBackgroundColorforCutting = (po: any) => {
  return getBackgroundColorCut(po?.WEE_END_DATE, po?.COMP_CUT_QTY, po?.SE_QTY,po?.CUT_SCH_QTY, 'cut');
}; 

const getBackgroundColorforStitching = (po: any) => {
  return getBackgroundColorCut(po?.WEE_END_DATE, po?.COM_STIT_QTY, po?.SE_QTY,po?.STIT_SCH_QTY, 'stit');
};

const getBackgroundColorforAssembly = (po: any) => { 
  return getBackgroundColorCut(po?.WEE_END_DATE, po?.COM_ASS_QTY, po?.SE_QTY,po?.ASS_SCH_QTY, 'ass');
};
...
const getrowcolor = (po: any) => {
  ...
  const isCutDone = cuttingstatus === 'cut-greenbackground';
  const isStitDone = stitchingstatus === 'stit-greenbackground';
  const isAssDone = assstatus === 'ass-greenbackground';
  const isCutNotStart = cuttingstatus === 'cut-blackbackground';
  const isStitNotStart = stitchingstatus === 'stit-blackbackground';
  const isAssNotStart = assstatus === 'ass-blackbackground';
  ...
  if (groupedData[seid]) {
    const { shipmentQtySum, orderQty} = 
      groupedData[seid];

      if((isCutDone===true && isStitDone===true && isAssDone===true && shipmentQtySum === orderQty ) || orderQty === shipmentQtySum ){
        return 'ass-greenbackground';
      }  
       if (!(isCutDelay ===true || isStitDelay===true || isAssDelay===true) && shipmentQtySum > 0 && currentDate<=crd ) {  
           return 'stit-orangebackground';
      }
      if((isCutDelay===true || isStitDelay===true || isAssDelay===true) || ( currentDate >= crd && shipmentQtySum !== orderQty) ) { 
          return 'cut-redbackground';
      } 
      if(isCutNotStart===true && isStitNotStart===true && isAssNotStart===true && shipmentQtySum === 0){ 
